...
export default function AuthButtons() {
  ...
  if (!user) {
    return (
      <div className="flex gap-2 text-sm">
        <Link className="pressable rounded-lg bg-slate-900 px-3 py-2 text-white" href="/login">로그인</Link>
        <Link className="pressable rounded-lg border border-slate-300 px-3 py-2" href="/signup">회원가입</Link>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <button
        className="pressable rounded-lg border border-slate-300 px-3 py-2"
        onClick={() => {
          localStorage.removeItem("smc_user");
